...
def get_features(X, X_train):
    customer_features = X_train.groupby('customerID').agg(
    {
    'InvoiceDate': ['count'],
    ...
    X_features = X_features.drop(['customerID', 'InvoiceDate', 'countryCode', 'InvoiceDate_DOW', 'InvoiceDate_Month', 
                                  'DueDate_DOW', 'DueDate_Month', 'Disputed', 'SettledDate', 'DaysToSettle', 'DaysLate',
                                  'DueDate', 'PaperlessDate'], axis=1)
